                ...
                <div class="card data-card mt-3">
                    <div class="card-header">
                        <h4>Hotel Booking Request
                             @can('hotel booking list csv export')
                            <a href="{{ route('hotel-booking.export.csv',['date_from'=>$request->date_from,'date_to'=>$request->date_to,'bill_to'=>$request->bill_to,'keyword'=>$request->keyword]) }}" class="btn btn-sm btn-cta ms-auto" data-bs-toggle="tooltip" title="Export data in CSV">
                        ...
                        <div class="table-responsive">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>Hotel</td>
                                    @endif
                                    <!--<td>{{ $item->seat_preference ?? '' }}</td>-->
                                    @if($item->hotel_type==1)
                                    <td>{{ $item->property->name ??''}}</td>
                                    @else
                                    <td>{{ $item->text ??''}}</td>
                                    @endif
                                    <td style="white-space:nowrap;">{{ date('d-m-Y',strtotime($item->checkin_date)) }}</td>
                                    <td style="white-space:nowrap;">{{ $item->checkin_time ? date('H:i', strtotime($item->checkin_time)) : '' }}</td>
                                    <td style="white-space:nowrap;">{{ date('d-m-Y H:i',strtotime($item->checkout_date)) }}</td>
                                    <td>{{ $item->guest_type }}</td>
                                    <td>{{ $item->guest_number }}</td>
                                     <td>{{ $item->food_preference ?? 'N/A' }}</td>
                                    ...
                                    <td style="white-space: nowrap;">
                                        @can('hotel booking details')
                                            <a href="{{ url('hotel-booking/details/'.$item->id) }}" class="btn btn-cta">
                                        ...
                                        @endcan
                                        @can('update hotel booking')
                                            <a href="{{ url('hotel-booking/'.$item->id.'/edit') }}" class="btn btn-cta">
