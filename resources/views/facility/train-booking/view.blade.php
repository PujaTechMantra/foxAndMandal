            ...
            <div class="col-md-12">
                ...
                <div class="card data-card">
                    <div class="card-header">
                        <h4 class="d-flex">Train/Bus Booking Detail
                            <a href="{{ url('train-booking/list') }}" class="btn btn-cta ms-auto">Back</a>
                            
                        </h4>
                    </div>
                    @can('train/bus booking status update')
                    <div class="card shadow-sm">
                        <div class="card-body">
                            <div class="btn-group" role="group" aria-label="Basic outlined example">
                                <a href="{{ url('train-booking/status/change/'.$data->id.'/1') }}"
                                   type="button"
                                   class="btn btn-outline-secondary btn-sm {{ $data->status == 1 ? 'active' : '' }}">
                                   Pending
                                </a>
                    
                                <a href="{{ url('train-booking/status/change/'.$data->id.'/2') }}"
                                   type="button"
                            ...
                            <div class="mt-3" id="remarksBox" style="display: none;">
                                <form method="GET" action="{{ url('train-booking/status/change/'.$data->id.'/4') }}">
                                    @csrf
                            ...
                            <div class="mt-3" id="pnrBox" style="display: none;">
                                <form method="GET" action="{{ url('train-booking/status/change/'.$data->id.'/3') }}">
                                    @csrf
                                    
                                    <div class="form-group">
                                        <label for="remarks">Enter PNR</label>
                                        <input type="text" name="pnr" class="form-control" value="{{$data->pnr}}" placeholder="Enter PNR No...">
                                    </div>
                                    
                                    @if($data->type==1)
                                    <div class="form-group">
                                        <label for="remarks">Enter Train Number</label>
                                        <input type="text" name="train_no" class="form-control" value="{{ $data->train_no }}" placeholder="Enter Train No...">
                                    </div>
                                    @else
                                    <div class="form-group">
                                        <label for="remarks">Enter Bus Number</label>
                                        <input type="text" name="train_no" class="form-control" value="{{ $data->train_no }}" placeholder="Enter Bus No...">
                                    </div>
                                    @endif
                                    <div class="form-group">
                                        <label for="remarks">Enter Dept. Date & Time</label>
                                        <input type="datetime-local" name="date_time" class="form-control" value="{{ $data->date_time }}" placeholder="Enter Dept. Date & Time...">
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <table class="table">
                                        ...
                                        <tr>
                                         ...
                                         <tr>
                                            ...
                                            <td>{{ $data->type == 1 ? 'Train' : ($data->type == 2 ? 'Bus' : '') }}</td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <td class="text-muted">Return Time : </td>
                                            <td>{{ $data->trip_type == 2 ? ($data->return_time ?? '') : '' }}</td>
                                        </tr>
                                       @if (count($formattedTravellers) > 0)
                                        ...
                                        @if($data->status==3)
                                            ...
                                            @if($data->type==1)
                                            <tr>
                                                <td class="text-muted">Train Number : </td>
                                                <td>{{ $data->train_no }}</td>
                                            </tr>
                                            @else
                                            <tr>
...
<script>
   ...
   $(document).ready(function() {
    ...
    $('#ticket').on('change', function() {
        ...
        $.ajax({
            url: "{{ route('train.upload.ticket', $data->id) }}",
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
