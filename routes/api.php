...
use App\Http\Controllers\Api\Fms\MatterCodeController;
...
Route::prefix('matter-codes')->group(function () {
    Route::get('/list', [MatterCodeController::class, 'index']);
    Route::post('/store', [MatterCodeController::class, 'store']);    
    Route::get('/suggest', [MatterCodeController::class, 'suggest']);
});
...
Route::get('/station/list/search', [UserController::class, 'stationListSearch']);

Route::get('/airport/list/search', [UserController::class, 'airportListSearch']);

//mail view

Route::get('/flight-mail/{id}', function ($id) {
    $flightBooking = \App\Models\FlightBooking::find($id);
    $user=\App\Models\User::where('id',$flightBooking->user_id)->first();
    $name=$user->name;

    if (!$flightBooking) {
        abort(404, 'Flight booking not found.');
    }

    return view('mail.flight-booking-mail', compact('flightBooking','name'));
});